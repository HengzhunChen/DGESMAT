...
% NOTE: For real wave functions, when number of grid points is even, 
% a post-process to take real part of complex number is needed, which has
% several ways to perform this operation. The following approach is 
% economic one and consistent with treatment for complex wave function.  

ng = domain.numGrid;
ngF = domain.numGridFine;

iF = [1: floor(ng(1)/2)+1, (floor(ng(1)/2)+2 :ng(1)) + ngF(1)-ng(1)];
jF = [1: floor(ng(2)/2)+1, (floor(ng(2)/2)+2 :ng(2)) + ngF(2)-ng(2)];
kF = [1: floor(ng(3)/2)+1, (floor(ng(3)/2)+2 :ng(3)) + ngF(3)-ng(3)];
