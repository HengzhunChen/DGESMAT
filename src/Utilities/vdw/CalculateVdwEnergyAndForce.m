function [EVdw, forceVdw] = CalculateVdwEnergyAndForce(domain, atomList, VDWType, XCType)
% CALCULATEVDWENERGYANDFORCE calculates van der Waals energy and force.
%
%    [EVdw, forceVdw] = CalculateVdwEnergyAndForce(domain, atomList, VDWType, XCType)
%    computes the van der Waals energy and force according to the 
%    type of VDW VDWType and type of exchange-correlation XCType over
%    domain with atoms in atomList.
%
%    See also Domain, Atom, HamiltionianKS/CalculatePseudopotential, 
%    HamiltionianKS/CalculatePseudopotential.
...
if VDWType == "DFT-D2"
    ...
    if XCType == "XC_GGA_XC_PBE"
      vdw_s = vdw_s_pbe;
    elseif XCType == "XC_HYB_GGA_XC_HSE06"
      vdw_s = vdw_s_hse;
    elseif XCType == "XC_HYB_GGA_XC_PBEH"
      vdw_s = vdw_s_pbe0;
    else
      error("Van der Waals DFT-D2 correction in only compatible with GGA-PBE, HSE06, and PBE0!");
    end
        ...
        for kk = -1 : 1
          ...
          for i = 1 : length(atomList)
            ...
            for j = 1 : i
              ...
              rx = atomList(i).pos(1) - atomList(j).pos(1) + ii * domainLength(1);
              ry = atomList(i).pos(2) - atomList(j).pos(2) + jj * domainLength(2);
              rz = atomList(i).pos(3) - atomList(j).pos(3) + kk * domainLength(3);
